...
Не забудьте про связи между таблицами. Например, в объявлении должна быть связь с пользователем и категорией.
...
Предыстория
На главной странице показываются карточки шести новых лотов. Это лоты, у которых не истёк срок их публикации, отсортированные от самых новых к старым. В прошлом задании вы написали SQL-запрос для получения таких записей. Сейчас вам будет необходимо заменить существующий массив с лотами на данные, полученные из MySQL по этому запросу.

Также в футере страницы находится список категорий. Пока это статичный список, его отдельные пункты прописаны прямо в вёрстке. В этом задании вы должны заменить его на данные из БД.
...
Не забудьте, что, согласно п. 5.1 спецификации, у сущности «Категория» есть символьный код, который нужно использовать в названии класса в главном меню: ul.promo__list .promo__item.promo__item--<символьный код>.

Как проверить результат
В прошлом задании вы перенесли массив со списком лотов в БД. Там теперь точно такая же информация — внешний вид главной страницы не должен поменяться. Но чтобы проверить, что всё работает корректно, добавьте руками в таблицу лотов новую запись, указав в качестве даты публикации текущее время. Затем обновите главную страницу и убедитесь, что в списке появилась новая карточка и она первая. Проделайте то же самое с категориями — удалите или добавьте новую.